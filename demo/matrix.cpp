...
static FLOAT sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrarg*sqrarg)  //平方
...
void Matrix::allocateMemory(const int32_t m_, const int32_t n_) {
	...
	val = (FLOAT**)malloc(m * sizeof(FLOAT*));//二级指针数组申请内存空间，只申请堆内存，未对其进行初始化
	val[0] = (FLOAT*)calloc(m*n, sizeof(FLOAT));//初始化堆内存  m*n个float类型大小的空间，空间的类型为float* ？？？
...
Matrix& Matrix::operator= (const Matrix &M) {
	...
	if (M.n > 0)
		for (int32_t i = 0; i < M.m; i++)
			memcpy(val[i], M.val[i], M.n * sizeof(FLOAT));//从 M.val[i]起始位置拷贝M.n * sizeof(FLOAT)个数据到val[i]中
	}
	return *this;
}
//获取子矩阵的数据
void Matrix::getData(FLOAT* val_, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
	...
	if (j2 == -1) j2 = n - 1;
	int32_t k = 0;
	for (int32_t i = i1; i <= i2; i++)//取矩阵i1行j1列数据到最后一个元素的数据
...
Matrix Matrix::diag(const Matrix &M) {
	if (M.m > 1 && M.n == 1) {
		...
		for (int32_t i = 0; i < M.m; i++)
			D.val[i][i] = M.val[i][0];
		return D;
	}//m（m>=2）行1列，把该列元素放到对角线元素的位置
	else if (M.m == 1 && M.n > 1) {
		Matrix D(M.n, M.n);
		for (int32_t i = 0; i < M.n; i++)
			D.val[i][i] = M.val[0][i];
		return D;
	}//1行n列元素，把该行元素放到对角线元素的位置
...
bool Matrix::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
	...
	FLOAT* vv = (FLOAT*)malloc(n * sizeof(FLOAT)); //存储每行的缩放比例，即每行的最大值分之一
	d = 1.0;
	for (i = 0; i < n; i++) { //循环行
		big = 0.0;
		for (j = 0; j < n; j++)//循环该行求最大值
...
ostream& operator<< (ostream& out, const Matrix& M) {
	if (M.m == 0 || M.n == 0) {
		...
		for (int32_t i = 0; i < M.m; i++) {
			for (int32_t j = 0; j < M.n; j++) {
				sprintf(buffer, "%12.7f ", M.val[i][j]);//sprintf的作用是将一个格式化的字符串输出到一个目的字符串buffer中，而printf是将一个格式化的字符串输出到屏幕。sprintf函数只比printf函数多第一个参数
				out << buffer;
			}
			if (i < M.m - 1)//输出一行之后进行换行
				out << endl;
		}
	}
	return out;
