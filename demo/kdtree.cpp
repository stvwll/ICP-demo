...
namespace kdtree {
	...
	//-----KDTreeResultVector类中三个函数的实现----------------保持堆的形式作为优化序列//
	...
	void KDTreeResultVector::push_element_and_heapify(KDTreeResult& e) {
		push_back(e); // 向结果集中添加一个元素
		push_heap(begin(), end()); // 调整插入元素的位置，保持原来的最大堆或最小堆。
	}
	...
	float KDTreeResultVector::replace_maxpri_elt_return_new_maxpri(KDTreeResult& e) {
		...
		pop_heap(begin(), end());//删除堆中一个元素，并调整为原来的最大堆或最小堆
		pop_back();
		
		push_back(e); // 插入一个新的元素
		push_heap(begin(), end());  // 堆化处理（保持最大堆或最小堆）

		return((*this)[0].dis);
	}
	//--------------KDTree类中函数的实现----------------//
	KDTree::KDTree(KDTreeArray& data_in, bool rearrange_in, int dim_in) : the_data(data_in),N(data_in.shape()[0]),dim(data_in.shape()[1]),sort_results(false),rearrange(rearrange_in),
		                                                                  root(NULL),data(NULL),ind(N) {//data_in.shape()[0] 数组的行数 data_in.shape()[1]数组的列数
		if (dim_in > 0)
			dim = dim_in;
		...
		if (rearrange) {
			...
			rearranged_data.resize(boost::extents[N][dim]);//为重排的数据元素分配内存空间，N行dim列

			// 重排数据
			for (int i = 0; i < N; i++) {
				for (int j = 0; j < dim; j++) {
					rearranged_data[i][j] = the_data[ind[i]][j];//ind是一个整型的向量容器 std::vector<int> ind
				}
			}
	...
	//使用详细的慢查找，通常用于调试，对于一个给定的查询向量qv，寻找离nn最近的点
