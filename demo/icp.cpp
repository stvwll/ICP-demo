...
Icp::Icp(double *M, const int32_t M_num, const int32_t dim)	:m_dim(dim), m_max_iter(200), m_min_delta(1e-4)
{
	// 二维点云还是三维点云dim
	if (dim != 2 && dim != 3) {
	...
	for (int32_t m = 0; m < M_num; m++)
		for (int32_t n = 0; n < dim; n++)
			m_kd_data[m][n] = (float)M[m*dim + n];//一个数据的所有维数都存储结束之后，再进行下一个数据的存储，此时换下一行开始存储
