...
// 两个三维点集的最小二乘拟合   Also see (3d part): "Least-Squares Fitting of Two 3-D Point Sets" (Arun, Huang and Blostein)
double IcpPointToPoint::fitStep(double *T, const int32_t T_num, Matrix &R, Matrix &t, const std::vector<int32_t> &active) {

	int i;
	int nact = (int)active.size();//向量中元素的个数
	...
	if (m_dim == 2) {
		...
		double r00 = R.val[0][0]; double r01 = R.val[0][1];//矩阵转化为向量，矩阵的每行代表一个点坐标，每行转化为一个向量
